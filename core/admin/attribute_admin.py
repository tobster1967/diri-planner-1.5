...
class AttributeAdmin(DynamicModelAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Value", {"fields": ("data_type", "value", "description")}),
        (
            "Tree Information",
            {
                "fields": ("tree_info",),
        ...
        match data_type:
            case "boolean":
                ...
                form.base_fields["value"] = forms.BooleanField(
                    required=False, initial=initial, help_text="Check for True, uncheck for False"
                )
            case "integer":
                # For integer type, use a number input
                form.base_fields["value"].widget = forms.NumberInput(attrs={"step": "1", "class": "vIntegerField"})
                form.base_fields["value"].help_text = "Enter an integer value"
            case "float":
                # For float type, use a number input with decimal support
                form.base_fields["value"].widget = forms.NumberInput(attrs={"step": "any", "class": "vFloatField"})
                form.base_fields["value"].help_text = "Enter a decimal value"
            case "date":
                # For date type, use a date input
                form.base_fields["value"].widget = forms.DateInput(attrs={"type": "date", "class": "vDateField"})
            ...
            case _:
                ...
                form.base_fields["value"].widget = forms.Textarea(attrs={"rows": 3, "class": "vLargeTextField"})
